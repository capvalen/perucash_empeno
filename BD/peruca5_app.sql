...
-- Tiempo de generación: 04-04-2017 a las 13:20:19
...
CREATE DEFINER=`peruca5`@`localhost` PROCEDURE `actualizarDatosCliente`(IN `apel` VARCHAR(100), IN `nomb` VARCHAR(100), IN `ddni` VARCHAR(8), IN `direc` VARCHAR(100), IN `corr` VARCHAR(100), IN `celu` VARCHAR(100), IN `idCli` INT)
    NO SQL
begin 
UPDATE `Cliente` SET 
`cliApellidos`=apel,
...
CREATE DEFINER=`peruca5`@`localhost` PROCEDURE `listarProductoPorId`(IN `idProd` INT)
    NO SQL
BEGIN
...
CREATE DEFINER=`peruca5`@`localhost` PROCEDURE `listarTodoProductosPorSuc`(IN `desde` INT, IN `hasta` INT, IN `idSuc` INT)
    NO SQL
begin
...
CREATE DEFINER=`peruca5`@`localhost` PROCEDURE `listarTodoProductosSinSuc`(IN `desde` INT, IN `hasta` INT)
    NO SQL
begin
SELECT p.idProducto, prodNombre, format( prodMontoEntregado,2) as prodMontoEntregado, 
...
CREATE DEFINER=`peruca5`@`localhost` PROCEDURE `listarUnUsuario`(IN `idUser` INT)
    NO SQL
...
CREATE DEFINER=`peruca5`@`localhost` PROCEDURE `returnTotalProductos`()
    NO SQL
begin
...
CREATE DEFINER=`peruca5`@`localhost` PROCEDURE `returnTotalProductosPorId`(IN `idSuc` INT)
    NO SQL
begin
...
(54, 'romero aliaga', 'juro dan', '14253600', 'manzanos 1545', 'qq@gmail.com', '245252'),
...
(10, 'celular huawei g6 l33', 67.2, 4, '2017-03-02', '2017-02-28', 'Se adelantó S/. 12.80 de S/. 80.00 el día 02/03/2017<br>', 92.8, 0, 10, b'1', '2017-02-06 13:52:49', 2, 1),
...
(17, 'laptop dell inspiracion 14 serie 3000', 336, 4, '2017-03-06', '2017-03-06', 'Se adelantó S/. 64.00 de S/. 400.00 el día 06/03/2017<br>funda+cargador+mause', 464, 0, 17, b'1', '2017-02-07 17:54:08', 2, 1),
...
(78, 'moto pulsar rs 200 amarillo+casco+tarjeta+soat', 2000, 4, '2017-02-24', '2017-03-24', '', 2320, 0, 73, b'1', '2017-02-24 19:26:11', 2, 1),
...
(87, 'laptop lenovo ideapad 510 i7', 900, 4, '2017-03-09', '2017-04-09', '', 1080, 0, 81, b'1', '2017-03-09 12:46:02', 2, 1),
...
(114, 'celular samsung j7', 250, 4, '2017-03-31', '2017-04-14', '', 270, 0, 106, b'1', '2017-03-31 19:43:38', 2, 1),
