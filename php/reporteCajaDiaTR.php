...
if($totalRow==0){
	...
	while($row = mysqli_fetch_array($sql, MYSQLI_ASSOC))
	{
		...
		echo "<tr> <th scope='row'>{$row['idCaja']}</th> <td class='mayuscula'>{$row['cajaFecha']}</td> <td class='mayuscula'>{$row['cliNombres']}</td> <td>{$row['descripcion']} <i class='icofont icofont-long-arrow-right'></i> <em>{$row['usuNombres']}</em></td> <td>S/. {$row['cajaValor']}</td><tr>";
		if($totalRow==$i){
