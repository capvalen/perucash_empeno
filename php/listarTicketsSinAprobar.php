...
while($row = mysqli_fetch_array($sql, MYSQLI_ASSOC))
{
	if($_COOKIE['ckPower']==1 || $_COOKIE['ckPower']==9 ){
		...
		if(in_array($row["idTipoProceso"], $directo)){
			$botones="<button class='btn btn-warning btn-outline btnSinBorde btnCobrarTicket' data-toggle='tooltip' title='Cobrar ticket'><i class='icofont icofont-money'></i></button>";
