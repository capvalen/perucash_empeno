...
if($totalRow==0){
	...
	while($row = mysqli_fetch_array($sql, MYSQLI_ASSOC))
	{
		...
		echo "<tr> <th scope='row'><a href='productos.php?idProducto={$row['idProducto']}'>#{$row['idProducto']}</a> </th> <td class='mayuscula'>{$row['tipoDescripcion']}</td> <td><em>{$row['usuNick']}</em></td> <td>S/. {$row['pagoMonto']}</td></tr>";
		if($totalRow==$i){
